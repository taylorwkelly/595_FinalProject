...
struct Gnuplot2dDataset::Data2d : public GnuplotDataset::Data
{
    ...
    std::string m_styleString;  //!< The plotting style to use for this dataset.
...
Gnuplot2dDataset::Data2d::Data2d(const std::string& title)
    : Data(title),
      ...
      m_styleString(m_defaultStringStyle),
                                          ...
                                          std::string& dataFileName) const
{
    ...
    if (m_title.size() && !(m_title == "Untitled"))
    {
        os << " title \"" << m_title << "\"";
    }
    if (m_styleString.size())
    {
        os << " " << m_styleString;
    }
    else
    {
        ...
        case LINES_POINTS:
            ...
            {
            case NONE:
                os << " with linespoints pointtype 7";
                break;
            case X:
                os << " with errorlines";
                break;
...
/// Default plot style string static instance
std::string Gnuplot2dDataset::m_defaultStringStyle = "";
...
void
Gnuplot2dDataset::SetStyle(const std::string& style)
{
    reinterpret_cast<Data2d*>(m_data)->m_styleString = style;
}
...
void
Gnuplot2dDataset::Invert()
{
    // for (Point XYpoint : reinterpret_cast<Data2d*>(m_data)->m_pointset)
    // {
    //     NS_LOG_UNCOND("X: " << XYpoint.x << " Y: " << XYpoint.y << "\n");
    // }
    std::reverse(reinterpret_cast<Data2d*>(m_data)->m_pointset.begin(),
                 reinterpret_cast<Data2d*>(m_data)->m_pointset.end());
    // for (Point XYpoint : reinterpret_cast<Data2d*>(m_data)->m_pointset)
    // {
    //     NS_LOG_UNCOND("X: " << XYpoint.x << " Y: " << XYpoint.y << "\n");
    // }
...
void
Gnuplot2dDataset::Add(double x, double y, std::string label)
{
    struct Point data;
    data.empty = false;
    data.x = x;
...
Gnuplot3dDataset::Data3d::PrintDataFile(std::ostream& os, bool generateOneOutputFile) const
{
    for (PointSet::const_iterator i = m_pointset.begin(); i != m_pointset.end(); ++i)
    {
        ...
        os << i->x << " " << i->y << " " << i->z << " " << i->label << std::endl;
...
void
Gnuplot3dDataset::Add(double x, double y, double z, std::string label)
{
    struct Point data;
    data.empty = false;
    data.x = x;
