...
for line in Lines:
    ...
    if(vals[0] == "position"):
        ...
        position = vals[1]
    if(vals[0] == "summary"):
        myDict[manager].append((position,vals[6],vals[9],vals[10], vals[11]))
...
def extract_statistics(data, out_avg, out_std, ps, standard, path):
    tempList = []
    for i in range(MAX_POSITION+1):
        tempList.append([])
    for pos, thru, pack_size, stand, pth in data:
        # print(pos, thru, pack_size)
        if pack_size == ps and stand == standard and int(pth) == int(path):
...
for ps in size:
  for path in pathLoss:
    for stand in standards:
      ...
      cara_high = []
      cara_std = []
      extract_statistics(myDict[minstrel],mins_avg,mins_std, ps, stand, path)
      # bound(mins_avg,mins_std,mins_high,mins_low)

      extract_statistics(myDict[aarf],aarf_avg,aarf_std, ps, stand, path)


      extract_statistics(myDict[cara],cara_avg,cara_std, ps, stand, path)
      ...
      plt.legend()
      # print(f"Distance vs Throughput,{pathType},Standard={stand}")
      plt.title(f"Distance vs Throughput,{pathType},Stand={stand},PackSize={ps}", loc="left", wrap=True)
      plt.grid()
      plt.tight_layout()
      plt.savefig(f"path-throughput_ps_{pathType}_{stand}.png")
