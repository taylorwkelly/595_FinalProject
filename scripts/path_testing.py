...
def extract_statistics(data, out_avg, out_std, ps, standard, path):
    ...
    for pos, thru, pack_size, stand, pth in data:
        ...
        if stand == standard and int(pth) == int(path):
...
for path in pathLoss:
  for stand in standards:
    ...
    cara_high = []
    cara_std = []
    extract_statistics(myDict[minstrel],mins_avg,mins_std, size, stand, path)
    # bound(mins_avg,mins_std,mins_high,mins_low)

    extract_statistics(myDict[aarf],aarf_avg,aarf_std, size, stand, path)


    extract_statistics(myDict[cara],cara_avg,cara_std, size, stand, path)
    ...
    plt.title(f"Distance vs Throughput, {pathType}, Standard={stand}", loc="left", wrap=True)
    plt.grid()
    plt.tight_layout()
